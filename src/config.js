...
const defaults = {
  ...
  INCREMENTAL_UPDATE_TIMES: '1', // 每进行多少次增量更新后执行一次全量更新  16代表4小时全量更新一次
...
export const config = {
  ...
  SERVER_PORT: parseInt(process.env.SERVER_PORT || defaults.SERVER_PORT, 10),
  ...
  MAX_RETRIES: parseInt(process.env.MAX_RETRIES || defaults.MAX_RETRIES, 10),
  ...
  SEARCH_SIZE: parseInt(process.env.SEARCH_SIZE || defaults.SEARCH_SIZE, 10),
  ...
  INCREMENTAL_UPDATE_TIMES: parseInt(
    process.env.INCREMENTAL_UPDATE_TIMES || defaults.INCREMENTAL_UPDATE_TIMES,
    10
  ),
  NPMJS_CONCURRENT_REQUESTS: parseInt(
    process.env.NPMJS_CONCURRENT_REQUESTS || defaults.NPMJS_CONCURRENT_REQUESTS,
    10
  )
}
