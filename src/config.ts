...
// 环境变量解析辅助函数
const toNumber = (input: string | undefined, fallback: number): number => {
  if (input == null) return fallback
  const n = Number(input)
  return Number.isFinite(n) ? n : fallback
}

const toBoolean = (input: string | undefined, fallback: boolean): boolean => {
  if (input == null) return fallback
...
export const config = {
  ...
  SERVER_PORT: toNumber(process.env.SERVER_PORT, defaults.SERVER_PORT),
  SCAN_CRON: process.env.SCAN_CRON || defaults.SCAN_CRON,
  REQUEST_TIMEOUT: toNumber(
    process.env.REQUEST_TIMEOUT,
    defaults.REQUEST_TIMEOUT
  ),
  MAX_RETRIES: toNumber(process.env.MAX_RETRIES, defaults.MAX_RETRIES),
  OPTIMAL_WORKERS_MULTIPLIER: toNumber(
  ...
  SEARCH_SIZE: toNumber(process.env.SEARCH_SIZE, defaults.SEARCH_SIZE),
  ...
  INCREMENTAL_UPDATE: toBoolean(
    process.env.INCREMENTAL_UPDATE,
    defaults.INCREMENTAL_UPDATE
  ),
  INCREMENTAL_UPDATE_TIMES: toNumber(
    process.env.INCREMENTAL_UPDATE_TIMES,
    defaults.INCREMENTAL_UPDATE_TIMES
  ),
  NPMJS_CONCURRENT_REQUESTS: toNumber(
    process.env.NPMJS_CONCURRENT_REQUESTS,
    defaults.NPMJS_CONCURRENT_REQUESTS
  )
}
